...
class Irumudi_by_receipt(models.Model):
    ...
    def _str_(self):
        return f"Irumudi {self.Start_Receipt_Number} to {self.End_Receipt_Number} - {self.Total_Amount}"
...
class Report(models.Model):
    ...
    upi_total = models.DecimalField(max_digits=10, decimal_places=2)
    expense_total = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    
    def __str__(self):
        return f"{self.date} - Rs. {self.grand_total} - Rs. {self.upi_total} - Rs. {self.expense_total}"
...
class Expense(models.Model):
    ...
    def __str__(self):
        return f"{self.date} - {self.expense_name} - Rs. {self.amount}"

# harsha_15-5-2025
class ghee_coconut_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    ...
    def __str__(self):
        return f"GC-{self.receipt_range_start} to GC-{self.receipt_range_end} | Count: {self.receipts_count} | Rs. {self.total_cost}"

class Maladharane_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    receipt_range_start = models.CharField(max_length=20)
    receipt_range_end = models.CharField(max_length=20)
    receipts_count = models.IntegerField()
    ...
    def __str__(self):
        return f"ML-{self.receipt_range_start} to ML-{self.receipt_range_end} | Count: {self.receipts_count} | Rs. {self.total_cost}"

class Irumudi_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    receipt_number = models.CharField(max_length=20)
    amount = models.DecimalField(max_digits=10, decimal_places=2)

    def __str__(self):
        return f"IR-{self.receipt_number} | Rs. {self.amount}"

class Donation_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    receipt_number = models.CharField(max_length=20)
    amount = models.DecimalField(max_digits=10, decimal_places=2)

    def __str__(self):
        return f"DN-{self.receipt_number} | Rs. {self.amount}"

class Materials_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    receipt_number = models.CharField(max_length=20)
    amount = models.DecimalField(max_digits=10, decimal_places=2)

    def __str__(self):
        return f"MT-{self.receipt_number} | Rs. {self.amount}"

class UPI_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    amount = models.DecimalField(max_digits=10, decimal_places=2)

    def __str__(self):
        return f"UPI | Rs. {self.amount}" 
        

class OB_new(models.Model):
    date = models.DateField(default=datetime.date.today)
    amount = models.DecimalField(max_digits=10, decimal_places=2)
